...
public class Clause {
	...
	public void addPrepositionPhrase() {// WORK
		...
		int prepI = (int) (Math.random() * posPreps(classVerb.getClassVerb()).split("--").length);// prepI = instance of
																									// prep in verb Def
		String prepS = ranPrep(posPreps(classVerb.getClassVerb()).split("--")[prepI].split("-")[0].split(",")); // prepS
																												// =
																												// preposition
																												// String
		NPPhraseSpec prepNP = nlgFactory.createNounPhrase(
				ranNoun(posPreps(classVerb.getClassVerb()).split("--")[prepI].split("-")[1].split(",")));// prepositional
																											// phrase
	...
	public static String ranPrep(String[] a) {
		...
		String g;
		for (int i = 0; i < preps.size(); i++) {//////// Goes through verbs
	...
	public static SPhraseSpec makeSentance() {
		...
		{
			int prepI = (int) (Math.random() * posPreps(v2.getClassVerb()).split("--").length);// prepI = instance of
																								// prep in verb Def
			String prepS = ranPrep(posPreps(v2.getClassVerb()).split("--")[prepI].split("-")[0].split(",")); // prepS =
																												// preposition
																												// String
			NPPhraseSpec prepNP = nlgFactory
					.createNounPhrase(ranNoun(posPreps(v2.getClassVerb()).split("--")[prepI].split("-")[1].split(",")));// prepositional
																														// phrase
			prepNP.setDeterminer(ranDet());
			pp = nlgFactory.createPrepositionPhrase();
			pp.addComplement(prepNP);
